...
resource "null_resource" "ansible-provision" {
  ...
  provisioner "local-exec" {
    command = "echo '${digitalocean_droplet.mongo-test1.ipv4_address} ${digitalocean_droplet.mongo-test1.name}' > ./etc/hosts"
  }
  provisioner "local-exec" {
    command = "echo '${digitalocean_droplet.mongo-test2.ipv4_address} ${digitalocean_droplet.mongo-test2.name}' >> ./etc/hosts"
  }

  provisioner "local-exec" {
    command = "echo '${digitalocean_droplet.mongo-test3.ipv4_address} ${digitalocean_droplet.mongo-test3.name}' >> ./etc/hosts"
  }
  provisioner "local-exec" {
